<template>
	<div
		class="load-profile-game mt-3 m-2 d-flex flex-column align-items-center mt-4"
	>
		<button
		...
		>
			<span>{{
				dataSave
					? "Ganancias guardadas al perfil: " +
					  profileStore.profile.data.username
					: "Guardar ganancia en el perfil"
			}}</span>
		</button>
	</div>
	...
	<div
		v-if="isLoading"
		class="loading mt-5 d-flex flex-column align-items-center gap-3"
	>
		<div class="spinner-border text-danger" role="status"></div>
		<div>
			<p>
				Guardando ganancias al perfil:
				{{ profileStore.profile.data.username }}
			</p>
		</div>
	</div>
